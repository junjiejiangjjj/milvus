...
func (t *addCollectionFunctionTask) PreExecute(ctx context.Context) error {
	...
	newFunc := &schemapb.FunctionSchema{}
	if err := proto.Unmarshal(t.FunctionSchema, newFunc); err != nil {
		log.Ctx(t.ctx).Error("AddFunctionTask, unmarshal function failed",
			zap.String("collectionName", t.GetCollectionName()), zap.Error(err))
		return fmt.Errorf("Cant not parse function info, error: %v", err)
	}
	if newFunc.Type == schemapb.FunctionType_BM25 {
	...
	newColl.Functions = append(coll.schema.CollectionSchema.Functions, newFunc)
