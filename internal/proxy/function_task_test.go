...
func (f *FunctionTaskSuite) TestAddCollectionFunctionTaskPreExecute() {
...

	{
		...
		req := &milvuspb.AddCollectionFunctionRequest{
			...
			CollectionName: "test_collection",
			CollectionID:   1,
			FunctionSchema: []byte("invalid schema"),
		}
		...
		globalMetaCache = cache
		err := task.PreExecute(ctx)
		f.ErrorContains(err, "Cant not parse function info")
	}

	{
		...
		schemaBytes, err := proto.Marshal(functionSchema)
		f.NoError(err)
