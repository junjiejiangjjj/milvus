...
func (t *searchTask) initAdvancedSearchRequest(ctx context.Context) error {
	...
	for index, subReq := range t.request.GetSubReqs() {
		...
		plan, queryInfo, offset, subIsIterator, _, searchType, err := t.tryGeneratePlan(subReq.GetSearchParams(), subReq.GetDsl(), subReq.GetExprTemplateValues(), t.request.GetFunctionScore() != nil || len(querynodeFunctionChains) > 0)
		if err != nil {
			return err
		}
...
func (t *searchTask) initSearchRequest(ctx context.Context) error {
	...
	plan, queryInfo, offset, isIterator, orderByFields, searchType, err := t.tryGeneratePlan(t.request.GetSearchParams(), t.request.GetDsl(), t.request.GetExprTemplateValues(), hasFunctionRerank(t.request))
	if err != nil {
		return err
	}
...
func (t *searchTask) tryGeneratePlan(params []*commonpb.KeyValuePair, dsl string, exprTemplateValues map[string]*schemapb.TemplateValue, functionRerank bool) (*planpb.PlanNode, *planpb.QueryInfo, int64, bool, []OrderByField, internalpb.SearchType, error) {
