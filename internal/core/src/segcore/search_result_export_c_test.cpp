...
TEST(SearchResultExport, ExportSearchResultAsArrowStream_EmptyGroupByLayout) {
    auto schema = std::make_shared<Schema>();
    auto pk_fid = schema->AddDebugField("pk", DataType::INT64);
    schema->set_primary_field_id(pk_fid);
...
TEST(SearchResultExport,
     ExportSearchResultAsArrowStream_EmptyMultiFieldGroupByLayout) {
    auto schema = std::make_shared<Schema>();
    auto pk_fid = schema->AddDebugField("pk", DataType::INT64);
    schema->set_primary_field_id(pk_fid);
    auto group_fid_1 = schema->AddDebugField("group1", DataType::INT64);
...
TEST(SearchResultExport,
     ExportSearchResultAsArrowStream_AllowsSingleJsonFieldWithScalarGroupBy) {
    auto schema = std::make_shared<Schema>();
    auto pk_fid = schema->AddDebugField("pk", DataType::INT64);
    schema->set_primary_field_id(pk_fid);
...
TEST(SearchResultExport,
     ExportSearchResultAsArrowStream_RejectsMultiJsonFieldGroupBy) {
    auto schema = std::make_shared<Schema>();
    auto pk_fid = schema->AddDebugField("pk", DataType::INT64);
    schema->set_primary_field_id(pk_fid);
...
TEST(SearchResultExport,
     ExportSearchResultAsArrowStream_ExtraFieldOrderMatchesRequest) {
    using namespace milvus;
    using namespace milvus::segcore;
    ...
    auto empty_reader_result = arrow::ImportRecordBatchReader(&empty_stream);
    ASSERT_TRUE(empty_reader_result.ok())
        << empty_reader_result.status().ToString();
    auto empty_batch_result = (*empty_reader_result)->Next();
    ...
    ArrowArrayStream non_empty_stream{};
    status = ExportSearchResultAsArrowStream(
    ...
    auto non_empty_reader_result =
        arrow::ImportRecordBatchReader(&non_empty_stream);
    ASSERT_TRUE(non_empty_reader_result.ok())
        << non_empty_reader_result.status().ToString();
    auto non_empty_batch_result = (*non_empty_reader_result)->Next();
...
TEST(SearchResultExport, ExportSearchResultAsArrowStream_ExtraFieldIds) {
    using namespace milvus;
    using namespace milvus::segcore;
