...
TEST(SearchResultExport, ExportSearchResultAsArrowStream_RejectsUnpreparedRows) {
...
TEST(SearchResultExport, ExportSearchResultAsArrowStream_RejectsMultiFieldGroupBy) {
    auto schema = std::make_shared<Schema>();
    auto pk_fid = schema->AddDebugField("pk", DataType::INT64);
    schema->set_primary_field_id(pk_fid);
    auto group_fid_1 = schema->AddDebugField("group1", DataType::INT64);
    auto group_fid_2 = schema->AddDebugField("group2", DataType::INT64);
    Plan plan(schema);
    plan.plan_node_ = std::make_unique<VectorPlanNode>();
    ...
    EXPECT_NE(status.error_code, 0);
    ASSERT_NE(status.error_msg, nullptr);
    EXPECT_NE(std::string(status.error_msg).find("exactly one group_by field"),
              std::string::npos)
        << status.error_msg;
    free(const_cast<char*>(status.error_msg));
}
...
// removed when the Go reduce pipeline replaced ReduceSearchResultsAndFillData
// in commit 52a5083ded).
// ---------------------------------------------------------------------------
...
// having unity_topK=2 → refine_topk = ceil(0.5 * max(2, 2)) = 1. k-way merge
// across segments keeps only the single best (0.95 in seg0); seg1 loses all
// of its candidates.
...
// TruncateToRefineTopk with mixed per-segment unity_topK: seg0 has 3
// candidates, seg1 has 1. refine_topk = ceil(1.0 * max(2, 3)) = 3. Global
...
// run segment->Search to get a real SearchResult, then drive the FULL
// ReduceHelper::Reduce() + Marshal() pipeline with refine-enabled forced
// on. This exercises PreReduce → TruncateToRefineTopk → RefineDistances →
// CalcDistByIDs → ApplyRefinedOrderForOneNQ → rest of Reduce → Marshal
// end-to-end, and verifies the marshalled blob can be parsed and has
// sane shape.
TEST(SearchResultExport, GlobalRefine_EndToEnd_ForcedSealedRefine) {
    ...
    using milvus::segcore::SearchResultDataBlobs;
    ...
    // --- 7. Run the full reduce pipeline with forced refine-enabled. ---
    //
    ...
    // --- 8. Parse marshalled blob and sanity-check shape/finiteness. ---
    auto* blobs =
    ...
    const auto& blob = blobs->blobs[0];
    ASSERT_TRUE(
        result_data.ParseFromArray(blob.data(), static_cast<int>(blob.size())));
    EXPECT_EQ(result_data.num_queries(), num_queries);
    EXPECT_EQ(result_data.top_k(), topK);
    ASSERT_EQ(result_data.topks_size(), num_queries);
    for (auto real_topk : result_data.topks()) {
        EXPECT_GT(real_topk, 0);
        EXPECT_LE(real_topk, topK);
    }
    for (int i = 0; i < result_data.scores_size(); ++i) {
        EXPECT_FALSE(std::isnan(result_data.scores(i)));
        EXPECT_FALSE(std::isinf(result_data.scores(i)));
    }

    delete blobs;  // GetSearchResultDataBlobs() releases ownership to caller.
}
