...
// sortEqualScoresOneNQ stable-sorts an equal-score run within [nq_begin, nq_end)
// by PK ASC, using in-place cyclic permutation. Handles optional element_level_
// and group_by_values_ fields.
static void
sortEqualScoresOneNQ(size_t nq_begin,
    ...
    while (start < nq_end) {
        ...
        if (end - start > 1) {
            ...
            const bool has_element_level = search_result->element_level_ &&
