...
CompactValidRows(SearchResult* search_result) {
    ...
    const bool has_element_indices = search_result->element_level_ &&
...
BuildSearchResultBatch(
    ...
    if (group_by_info.enabled) {
        AssertInfo(search_result->group_by_values_.has_value(),
                   ...
                   "group_by_values_");
        AssertInfo(
            search_result->group_by_values_.value().size() == total_valid,
...
ExportSearchResultAsArrowStream(CSearchResult c_search_result,
    ...
    try {
        ...
        if (search_result->unity_topK_ == 0 ||
            ...
            return ExportEmptyBatches(plan, extra_field_ids, num_extra_fields,
                                      search_result->element_level_, nq,
                                      out_stream);
        }

        CompactValidRows(search_result);

        // After CompactValidRows, all invalid (-1) offsets are removed. If no
        // valid rows remain, fall back to the empty-batch path so the $id
        // column gets its type from the schema (not from the uninitialized
        // pk_type_ which defaults to NONE and would produce a utf8 column).
