...
// Keep allocation failures retriable through the C Data export boundary.
CStatus
ArrowExportFailure(const arrow::Status& status) {
    return milvus::FailureCStatus(status.IsOutOfMemory()
...
FieldDataToArrow(const std::string& field_name,
    ...
    if (scalars.has_int_data()) {
        // Protobuf stores INT8/INT16/INT32 in int_data. Materialize the
        // declared Milvus type, matching the Go Function Chain converter.
        if (field_data.type() == milvus::proto::schema::DataType::Int8) {
            ARROW_ASSIGN_OR_RAISE(
                auto arr,
        ...
        if (field_data.type() == milvus::proto::schema::DataType::Int16) {
            ARROW_ASSIGN_OR_RAISE(
                auto arr,
                BuildFixedWidthArray<arrow::Int16Builder>(
                    scalars.int_data().data(), field_data, total_valid));
            return std::make_pair(arrow::field(field_name, arrow::int16()), arr);
        }
        ARROW_ASSIGN_OR_RAISE(
            auto arr,
