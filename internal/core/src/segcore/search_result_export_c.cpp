...
ResolveGroupByArrowType(milvus::query::Plan* plan) {
    ...
    AssertInfo(search_info.group_by_field_ids_.size() == 1,
               "Go reduce Arrow export supports exactly one group_by field, got {}",
    ...
    if (search_info.json_type_.has_value()) {
        ...
        auto& field_meta =
            plan->schema_->operator[](search_info.group_by_field_ids_[0]);
        ...
        // so the Arrow column is utf8 — the Go-side exporter handles
        // utf8 natively. The caller fixes up the proto DataType to
        // Geometry / the real json_type afterward.
        if (dt == milvus::DataType::JSON || dt == milvus::DataType::GEOMETRY) {
    ...
    if (scalars.has_bool_data()) {
        ARROW_ASSIGN_OR_RAISE(auto arr,
                              BuildFixedWidthArray<arrow::BooleanBuilder>(
                                  scalars.bool_data().data(), total_valid));
    ...
    if (scalars.has_int_data()) {
        ARROW_ASSIGN_OR_RAISE(auto arr,
                              BuildFixedWidthArray<arrow::Int32Builder>(
                                  scalars.int_data().data(), total_valid));
    ...
    if (scalars.has_long_data()) {
        ARROW_ASSIGN_OR_RAISE(auto arr,
                              BuildFixedWidthArray<arrow::Int64Builder>(
                                  scalars.long_data().data(), total_valid));
    ...
    if (scalars.has_float_data()) {
        ARROW_ASSIGN_OR_RAISE(auto arr,
                              BuildFixedWidthArray<arrow::FloatBuilder>(
                                  scalars.float_data().data(), total_valid));
    ...
    if (scalars.has_double_data()) {
        ARROW_ASSIGN_OR_RAISE(auto arr,
                              BuildFixedWidthArray<arrow::DoubleBuilder>(
                                  scalars.double_data().data(), total_valid));
...
BuildSearchResultBatch(
    ...
    if (group_by_info.enabled) {
        ...
        AssertInfo(
            ...
            composite.size(),
            total_valid);
        std::vector<milvus::GroupByValueType> first_field_values;
        first_field_values.reserve(composite.size());
        for (const auto& key : composite) {
            AssertInfo(key.Size() == 1,
                       "Go reduce Arrow export supports exactly one group_by value per row, got {}",
                       key.Size());
            first_field_values.push_back(key[0]);
        }
        ARROW_ASSIGN_OR_RAISE(
            auto gb_arr,
            BuildGroupByArray(first_field_values, group_by_info.milvus_type));
        fields.push_back(arrow::field("$group_by", group_by_info.arrow_type));
        arrays.push_back(gb_arr);
    }
