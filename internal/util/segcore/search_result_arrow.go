...
func ExportSearchResultAsArrowStream(ctx context.Context, result *SearchResult, plan *SearchPlan, extraFieldIDs []int64) (arrio.Reader, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	if result == nil {
		return nil, merr.WrapErrParameterInvalidMsg("nil search result")
	}
	...
	if err != nil {
		return nil, merr.WrapErrServiceInternal("failed to import Arrow RecordBatchReader", err.Error())
	}
...
func newCRecordBatchReader(stream *C.struct_ArrowArrayStream) (*cRecordBatchReader, error) {
	...
	if reader.arr == nil {
		...
		return nil, merr.WrapErrServiceInternal("failed to allocate ArrowArray")
	}
...
func (r *cRecordBatchReader) ensureSchema() error {
	if r.schema != nil {
		return nil
	}
	if r.stream == nil {
