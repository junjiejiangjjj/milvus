...
// fill_pk only.
//
// Mutates the passed SearchResults in place.
func PrepareSearchResultsForExport(
	...
	if plan == nil || plan.cSearchPlan == nil {
		return merr.WrapErrParameterInvalidMsg("nil search plan")
	}
	if placeholderGroup == nil {
		return merr.WrapErrParameterInvalidMsg("nil placeholder group")
	}
	if len(searchResults) == 0 {
		return merr.WrapErrParameterInvalidMsg("empty search results")
	}
	if len(sliceNQs) == 0 || len(sliceNQs) != len(sliceTopKs) {
		return merr.WrapErrParameterInvalidMsg("unaligned slice nqs (%d) and topks (%d)",
	...
	for i, r := range searchResults {
		if r == nil {
			return merr.WrapErrParameterInvalidMsg("nil search result at index %d", i)
		}
