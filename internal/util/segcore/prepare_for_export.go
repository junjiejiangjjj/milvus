...
func PrepareSearchResultsForExport(
	...
	if placeholderGroup == nil {
		return errors.New("nil placeholder group")
	}
	if len(searchResults) == 0 {
		return errors.New("empty search results")
	}
	if len(sliceNQs) == 0 || len(sliceNQs) != len(sliceTopKs) {
		return errors.Newf("unaligned slice nqs (%d) and topks (%d)",
	...
	for i, r := range searchResults {
		if r == nil {
			return errors.Newf("nil search result at index %d", i)
		}
	...
	if err := ConsumeCStatusIntoError(&status); err != nil {
		return errors.Wrap(err, "PrepareSearchResultsForExport failed")
	}
	return nil
}
