...
func ComputeScorerScoresOnChunkedOffsets(segment CSegment, searchReq *SearchRequest, scoreFunction *planpb.ScoreFunction, offsets *arrow.Chunked) (*arrow.Chunked, error) {
	...
	if searchReq == nil || searchReq.plan == nil {
		return nil, errors.New("search request or plan is nil")
	}
	if scoreFunction == nil {
		return nil, errors.New("score function is nil")
	}
	if offsets == nil {
		return nil, errors.New("offsets is nil")
	}
	if offsets.DataType().ID() != arrow.INT64 {
		return nil, fmt.Errorf("offset column must be Int64, got %s", offsets.DataType())
	}
	...
	if len(scoreFunctionBlob) == 0 {
		return nil, errors.New("empty score function")
	}
	...
	if scorePtrArray == nil || hasScorePtrArray == nil {
		...
		return nil, errors.New("failed to allocate boost score pointer arrays")
	}
	...
	for chunkIdx, chunk := range offsets.Chunks() {
		...
		if !ok {
			return nil, fmt.Errorf("offset chunk %d must be Int64, got %T", chunkIdx, chunk)
		}
		if offsetChunk.NullN() > 0 {
			return nil, fmt.Errorf("offset chunk %d contains null", chunkIdx)
		}
	...
	status := C.ComputeScorerScoresOnOffsetChunks(
		...
		unsafe.Pointer(&scoreFunctionBlob[0]),
		C.int64_t(len(scoreFunctionBlob)),
		&offsetArrays[0],
		&offsetSchemas[0],
		C.int64_t(numChunks),
		...
		C.uint64_t(searchReq.entityTTLPhysicalTime),
		(**C.float)(unsafe.Pointer(scorePtrArray)),
		(**C.bool)(unsafe.Pointer(hasScorePtrArray)),
	)
