...
func (o *GroupByOp) processChunk(ctx *types.FuncContext, input *DataFrame, chunkIdx int) (*chunkResult, error) {
	...
	// Step 4: Sort groups by group score DESC, with tiebreaking:
...
// sortAndLimitGroup sorts rows within a group by score DESC (ties broken by id ASC)
// and keeps top groupSize.
func (o *GroupByOp) sortAndLimitGroup(g *group) {
	// Sort indices and scores together by score DESC, id ASC for tiebreaking
	n := len(g.rowIndices)
	indices := make([]int, n)
	for i := 0; i < n; i++ {
		indices[i] = i
