...
func NewRoundDecimalExpr(decimal int64) (*RoundDecimalExpr, error) {
	if decimal < 0 || decimal > 6 {
		return nil, fmt.Errorf("round_decimal: decimal must be in range [0, 6], got %d", decimal)
	}
...
func (e *RoundDecimalExpr) Execute(ctx *types.FuncContext, inputs []*arrow.Chunked) ([]*arrow.Chunked, error) {
	if len(inputs) != 1 {
		return nil, fmt.Errorf("round_decimal: expected 1 input column, got %d", len(inputs))
	}
	...
	for chunkIdx := 0; chunkIdx < numChunks; chunkIdx++ {
		...
		if !ok {
			...
			return nil, fmt.Errorf("round_decimal: input chunk %d must be Float32, got %T", chunkIdx, scoreCol.Chunk(chunkIdx))
		}
