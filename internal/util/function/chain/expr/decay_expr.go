...
// When decay is too close to 1, log(decay) approaches 0 causing numerical instability.
...
func NewDecayExpr(function string, origin, scale, offset, decay float64) (*DecayExpr, error) {
	...
	expr := &DecayExpr{
		BaseExpr: *NewBaseExpr("decay", nil),
		function: function,
		origin:   origin,
		scale:    scale,
		offset:   offset,
...
func NewDecayExprFromParams(params map[string]interface{}) (chain.FunctionExpr, error) {
...
func (d *DecayExpr) IsRunnable(stage string) bool {
	switch stage {
	case chain.ChainTypeIngestion, chain.ChainTypeL2Rerank, chain.ChainTypeL1Rerank,
		chain.ChainTypeL0Rerank, chain.ChainTypePreProcess, chain.ChainTypePostProcess:
		return true
	default:
		return false
	}
...
func (d *DecayExpr) Execute(ctx *chain.FuncContext, inputs []*arrow.Chunked) ([]*arrow.Chunked, error) {
	if len(inputs) != 2 {
...
func (d *DecayExpr) processChunk(ctx *chain.FuncContext, inputChunk arrow.Array, scoreChunk *array.Float32) (arrow.Array, error) {
...
func init() {
	chain.RegisterFunction("decay", NewDecayExprFromParams)
}
