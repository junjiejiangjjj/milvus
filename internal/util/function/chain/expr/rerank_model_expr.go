...
func NewModelExpr(provider rerank.ModelProvider, queries []string) (*ModelExpr, error) {
	...
	if len(queries) == 0 {
		return nil, fmt.Errorf("model: queries must not be empty")
	}
	return &ModelExpr{
...
func (m *ModelExpr) Execute(ctx *types.FuncContext, inputs []*arrow.Chunked) ([]*arrow.Chunked, error) {
	if len(inputs) != 1 {
		return nil, fmt.Errorf("model: expected 1 input column (text), got %d", len(inputs))
	}
	...
	if len(m.queries) != numChunks {
		return nil, fmt.Errorf("model: queries count (%d) != nq count (%d)", len(m.queries), numChunks)
	}
...
func (m *ModelExpr) processChunk(ctx *types.FuncContext, chunk arrow.Array, query string) (arrow.Array, error) {
	...
	if !ok {
		return nil, fmt.Errorf("model: input column must be String/VarChar, got %T", chunk)
	}
...
func (m *ModelExpr) rerankBatch(ctx context.Context, query string, texts []string) ([]float32, error) {
	...
	for i := 0; i < len(texts); i += maxBatch {
		...
		if len(batchScores) != end-i {
			return nil, fmt.Errorf("model: rerank service returned %d scores for %d docs", len(batchScores), end-i)
		}
