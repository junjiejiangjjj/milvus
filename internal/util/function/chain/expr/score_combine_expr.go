...
const (
	...
	ModeKey       = "mode"        // string - combine mode
	WeightsKey    = "weights"     // []float64 - optional weights array
	InputCountKey = "input_count" // int - number of input columns

	// Mode values
	ModeMultiply = "multiply" // multiply all scores
	ModeSum      = "sum"      // sum all scores
	ModeMax      = "max"      // take maximum
	ModeMin      = "min"      // take minimum
	ModeAvg      = "avg"      // average
	ModeWeighted = "weighted" // weighted sum (requires weights parameter)
)
...
func NewScoreCombineExprFromParams(params map[string]interface{}) (chain.FunctionExpr, error) {
...
func (s *ScoreCombineExpr) Name() string {
...
func (s *ScoreCombineExpr) IsRunnable(stage string) bool {
...
func (s *ScoreCombineExpr) Execute(ctx *chain.FuncContext, inputs []*arrow.Chunked) ([]*arrow.Chunked, error) {
...
func (s *ScoreCombineExpr) processChunk(ctx *chain.FuncContext, inputs []*arrow.Chunked, chunkIdx int) (arrow.Array, error) {
...
func init() {
	chain.RegisterFunction("score_combine", NewScoreCombineExprFromParams)
}
