...
func (f *BenchFilterFunction) Execute(ctx *FuncContext, inputs []*arrow.Chunked) ([]*arrow.Chunked, error) {
	col := inputs[0]
...
func (f *BenchScoreTransformFunction) Execute(ctx *FuncContext, inputs []*arrow.Chunked) ([]*arrow.Chunked, error) {
	col := inputs[0]
...
func BenchmarkSelectOp(b *testing.B) {
	...
	for _, bc := range benchCases {
		b.Run(bc.name, func(b *testing.B) {
			...
			selectCols := []string{IDFieldName, ScoreFieldName}
			if bc.selectCols >= 3 {
			...
			chain := NewFuncChain().Select(selectCols...)
...
func BenchmarkFilterOp(b *testing.B) {
	...
	for _, bc := range benchCases {
		b.Run(bc.name, func(b *testing.B) {
			...
			chain := NewFuncChain().
				Map(&BenchFilterFunction{threshold: bc.threshold}, []string{ScoreFieldName}, []string{"_filter"}).
				Filter("_filter")
...
func BenchmarkSortOp(b *testing.B) {
	...
	for _, bc := range benchCases {
		b.Run(bc.name, func(b *testing.B) {
			...
			chain := NewFuncChain().Sort("int64_field", bc.desc)
...
func BenchmarkLimitOp(b *testing.B) {
	...
	for _, bc := range benchCases {
		b.Run(bc.name, func(b *testing.B) {
			...
			var chain *FuncChain
			if bc.offset > 0 {
				chain = NewFuncChain().LimitWithOffset(bc.limit, bc.offset)
			} else {
				chain = NewFuncChain().Limit(bc.limit)
			}
...
func BenchmarkMapOp(b *testing.B) {
	...
	for _, bc := range benchCases {
		b.Run(bc.name, func(b *testing.B) {
			...
			chain := NewFuncChain().Map(&BenchScoreTransformFunction{multiplier: 2.0}, []string{ScoreFieldName}, []string{"transformed_score"})
...
func BenchmarkChain_MapFilterSelect(b *testing.B) {
	...
	for _, bc := range benchCases {
		b.Run(bc.name, func(b *testing.B) {
			...
			chain := NewFuncChain().
				Map(&BenchScoreTransformFunction{multiplier: 1.5}, []string{ScoreFieldName}, []string{"transformed_score"}).
				Map(&BenchFilterFunction{threshold: 0.5}, []string{ScoreFieldName}, []string{"_filter"}).
				Filter("_filter").
				Select(IDFieldName, ScoreFieldName, "transformed_score", "int64_field")
...
func BenchmarkChain_FullPipeline(b *testing.B) {
	...
	for _, bc := range benchCases {
		b.Run(bc.name, func(b *testing.B) {
			...
			// Full pipeline: Map -> Map -> Filter -> Sort -> Limit -> Select
			chain := NewFuncChain().
				Map(&BenchScoreTransformFunction{multiplier: 2.0}, []string{ScoreFieldName}, []string{"transformed_score"}).
				Map(&BenchFilterFunction{threshold: 0.3}, []string{ScoreFieldName}, []string{"_filter"}).
				Filter("_filter").
				Sort("transformed_score", true).
				Limit(50).
				Select(IDFieldName, "transformed_score", "int64_field", "float64_field")
	...
	for _, bc := range benchCases {
		b.Run(bc.name, func(b *testing.B) {
			...
			chain := NewFuncChain().
				...
				Sort(ScoreFieldName, true).
				Limit(50).
				Select(IDFieldName, ScoreFieldName, "int64_field")
			...
			for i := 0; i < b.N; i++ {
				...
				// Export
				_, err = result.ToSearchResultData()
				if err != nil {
					b.Fatal(err)
				}
