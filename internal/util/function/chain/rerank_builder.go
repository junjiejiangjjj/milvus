...
func buildWeightedChain(fc *FuncChain, funcSchema *schemapb.FunctionSchema, searchMetrics []string) (sortDescending bool, err error) {
	...
	if len(weights) == 0 {
		return true, fmt.Errorf("rerank_builder: weighted reranker requires weights parameter")
	}

	if len(weights) != len(searchMetrics) {
		return true, fmt.Errorf("rerank_builder: weights count %d != search count %d", len(weights), len(searchMetrics))
	}
...
func buildModelChain(fc *FuncChain, collSchema *schemapb.CollectionSchema, funcSchema *schemapb.FunctionSchema, searchMetrics []string, searchParams *SearchParams) error {
	...
	if len(funcSchema.InputFieldNames) != 1 {
		return fmt.Errorf("rerank_builder: model reranker requires exactly 1 input field, got %d", len(funcSchema.InputFieldNames))
	}
	inputField := funcSchema.InputFieldNames[0]
	...
	// Add Merge (use Max strategy since model replaces all scores)
	fc.Merge(MergeStrategyMax,
		WithMetricTypes(searchMetrics),
		WithNormalize(true))

	// Create and add ModelExpr
	modelExpr, err := expr.NewModelExpr(provider, queries)
	if err != nil {
		return fmt.Errorf("rerank_builder: %w", err)
	}
	...
	for _, param := range funcSchema.Params {
		if param.Key == queryKeyName {
			...
			if err := json.Unmarshal([]byte(param.Value), &queries); err != nil {
				return nil, fmt.Errorf("rerank_builder: parse rerank params [queries] failed: %w", err)
			}
			if len(queries) == 0 {
				return nil, fmt.Errorf("rerank_builder: rerank queries must not be empty")
			}
			return queries, nil
		}
	}
	return nil, fmt.Errorf("rerank_builder: rerank function missing required param: queries")
}
...
func validateVarcharInputField(collSchema *schemapb.CollectionSchema, fieldName string) error {
	for _, field := range collSchema.Fields {
		if field.Name == fieldName {
			...
			return fmt.Errorf("rerank_builder: model input field %s must be VarChar, got %s", fieldName, field.DataType.String())
		}
	}
...
func GetRerankNameFromFuncScore(funcScore *schemapb.FunctionScore) string {
	...
	return GetRerankName(funcScore.Functions[0])
}
