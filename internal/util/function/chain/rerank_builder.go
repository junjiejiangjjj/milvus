...
func buildWeightedChain(fc *FuncChain, funcSchema *schemapb.FunctionSchema, searchMetrics []string) (sortDescending bool, err error) {
	...
	for _, weight := range weights {
		if weight < 0 || weight > 1 {
			return true, merr.WrapErrParameterInvalidMsg("rank param weight should be in range [0, 1]")
		}
	}
...
func buildDecayChain(fc *FuncChain, collSchema *schemapb.CollectionSchema, funcSchema *schemapb.FunctionSchema, searchMetrics []string) error {
	...
	strategy, _, decayParams, err := parseDecayParams(funcSchema)
	if err != nil {
		return err
	}
	...
	// Always normalize scores so that $score is "higher = more relevant" before
	// multiplying with the decay factor (which is also "higher = better").
	// Without normalization, distance-based metrics (L2) break the multiply semantics.
	fc.Merge(strategy,
		WithMetricTypes(searchMetrics),
		WithNormalize(true))
