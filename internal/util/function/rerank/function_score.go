...
func createFunction(collSchema *schemapb.CollectionSchema, funcSchema *schemapb.FunctionSchema, pkType schemapb.DataType) (Reranker, error) {
	...
	case decayFunctionName:
		rerankFunc, newRerankErr = newDecayFunction(collSchema, funcSchema, pkType)
	default:
...
func NewFunctionScore(collSchema *schemapb.CollectionSchema, funcScoreSchema *schemapb.FunctionScore) (*FunctionScore, error) {
	...
	for _, field := range collSchema.Fields {
		if field.IsPrimaryKey {
			funcScore.pkType = field.DataType
		}
	}

	var err error
	if funcScore.reranker, err = createFunction(collSchema, funcScoreSchema.Functions[0], funcScore.pkType); err != nil {
		return nil, err
	}
