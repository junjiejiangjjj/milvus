...
func newRRFFunction(collSchema *schemapb.CollectionSchema, funcSchema *schemapb.FunctionSchema) (Reranker, error) {
	...
	base, err := newRerankBase(collSchema, funcSchema, rrfName, true, pkType)
	if err != nil {
		return nil, err
	}
...
func (rrf *RRFFunction[T]) processOneSearchData(ctx context.Context, searchParams *SearchParams, cols []*columns) *IDScores[T] {
	...
	for _, col := range cols {
		...
		for idx, id := range ids {
			...
			if _, ok := rrfScores[id]; !ok {
			}
		}
	}
	return newIDScores(rrfScores, searchParams)
}
...
func (rrf *RRFFunction[T]) Process(ctx context.Context, searchParams *SearchParams, inputs *rerankInputs) (*rerankOutputs, error) {
	...
	for _, cols := range inputs.data {
		idScore := rrf.processOneSearchData(ctx, searchParams, cols)
