...
const (
	...
	endpointKeyName string = "endpoint"
...
func newVllmProvider(params []*commonpb.KeyValuePair) (modelProvider, error) {
...
func newTeiProvider(params []*commonpb.KeyValuePair) (modelProvider, error) {
	endpoint, maxBatch, err := parseParams(params)
	if err != nil {
		return nil, err
	}
	...
	for _, param := range params {
		if strings.ToLower(param.Key) == providerParamName {
			...
			case vllmProviderName:
				return newVllmProvider(params)
			case teiProviderName:
				return newTeiProvider(params)
			default:
