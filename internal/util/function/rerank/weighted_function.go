...
func newWeightedFunction(collSchema *schemapb.CollectionSchema, funcSchema *schemapb.FunctionSchema) (Reranker, error) {
	...
	base, err := newRerankBase(collSchema, funcSchema, weightedName, true, pkType)
	if err != nil {
		return nil, err
	}
	...
	for _, param := range funcSchema.Params {
		if strings.ToLower(param.Key) == WeightsParamsKey {
	...
	if pkType == schemapb.DataType_Int64 {
		return &WeightedFunction[int64]{RerankBase: *base, weight: weights}, nil
	} else {
		return &WeightedFunction[string]{RerankBase: *base, weight: weights}, nil
	}
}

func (weighted *WeightedFunction[T]) processOneSearchData(ctx context.Context, searchParams *SearchParams, cols []*columns) (*IDScores[T], error) {
	...
	for i, col := range cols {
		...
		for j, id := range ids {
			if score, ok := weightedScores[id]; !ok {
				weightedScores[id] = weighted.weight[i] * col.scores[j]
			} else {
				weightedScores[id] = score + weighted.weight[i]*col.scores[j]
			}
		}
	}
...
func (weighted *WeightedFunction[T]) Process(ctx context.Context, searchParams *SearchParams, inputs *rerankInputs) (*rerankOutputs, error) {
	...
	for _, cols := range inputs.data {
		...
		idScore, err := weighted.processOneSearchData(ctx, searchParams, cols)
		if err != nil {
			return nil, err
		}
