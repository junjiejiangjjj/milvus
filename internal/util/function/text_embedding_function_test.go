...
func createRunner(url string, schema *schemapb.FieldSchema) (*OpenAIEmbeddingProvider, error) {
	return NewOpenAIEmbeddingProvider(schema, &schemapb.FunctionSchema{
		Name:           "test",
...
func (s *TextEmbeddingFunctionSuite) TestEmbedding() {
	...
	defer ts.Close()
	runner, err := createRunner(ts.URL, s.schema.Fields[2])
	s.NoError(err)
	{
		data := []string{"sentence"}
		ret, err2 := runner.CallEmbedding(data, false)
		s.NoError(err2)
		s.Equal(1, len(ret))
		s.Equal(4, len(ret[0]))
	...
	}
	{
		...
		ret, _ := runner.CallEmbedding(data, false)
		s.Equal([][]float32{{0.0, 0.1, 0.2, 0.3}, {1.0, 1.1, 1.2, 1.3}, {2.0, 2.1, 2.2, 2.3}}, ret)
	}
}

func (s *TextEmbeddingFunctionSuite) TestEmbeddingDimNotMatch() {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		var res openai.EmbeddingResponse
		res.Object = "list"
		res.Model = "text-embedding-3-small"
	...
	_, err2 := runner.CallEmbedding(data, false)
	s.Error(err2)
}

func (s *TextEmbeddingFunctionSuite) TestEmbeddingNubmerNotMatch() {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		var res openai.EmbeddingResponse
		res.Object = "list"
		res.Model = "text-embedding-3-small"
...
func (s *TextEmbeddingFunctionSuite) TestRunnerParamsErr() {
	...
	// 	_, err := NewOpenAIEmbeddingProvider(schema.Fields[2], &schemapb.FunctionSchema{
	// 		Name:           "test",
	// 		Type:           schemapb.FunctionType_Unknown,
	// 		InputFieldIds:  []int64{101},
	// 		OutputFieldIds: []int64{102},
	...
	// outputfield miss
	// {
	// 	_, err := NewOpenAIEmbeddingProvider(s.schema, &schemapb.FunctionSchema{
	// 		Name:           "test",
	// 		Type:           schemapb.FunctionType_Unknown,
	// 		InputFieldIds:  []int64{101},
