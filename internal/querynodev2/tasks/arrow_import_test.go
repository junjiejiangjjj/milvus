...
func TestDataFrameFromArrowReader_ReleasesRetainedChunksOnMetadataError(t *testing.T) {
	...
	_, err := dataFrameFromArrowReader(reader)
	require.Error(t, err)
	assert.True(t, reader.released, "reader Release should be called after import error")
