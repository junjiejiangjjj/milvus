...
func (t *SearchTask) Execute() error {
	...
	if err := segcore.PrepareSearchResultsForExport(
		t.ctx,
		searchReq.Plan(),
	...
	if err := t.executeGoReduce(segDFs, results, searchReq, metricType, tr, relatedDataSize); err != nil {
		return err
	}
	...
	// master branch's ReduceSearchResultsAndFillData boundary so A/B
