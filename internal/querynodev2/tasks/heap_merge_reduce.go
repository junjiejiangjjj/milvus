...
func heapMergeReduce(
	...
	return heapMergeReduceImpl(pool, inputs, topK, groupByOpts, numChunks, hasGroupBy, isStringPK)
}
...
func resolveInputCols(inputs []*chain.DataFrame, hasGroupBy bool, groupByFieldName string) []inputCols {
	...
	for i, df := range inputs {
		...
		if hasGroupBy {
			cols[i].groupBy = df.Column(groupByFieldName)
		}
	}
	return cols
}
...
func heapMergeReduceImpl(
	...
	cols := resolveInputCols(inputs, hasGroupBy, groupByFieldName)
	...
	for chunkIdx := 0; chunkIdx < numChunks; chunkIdx++ {
		...
		if isStringPK {
			resultSources = mergeChunkStringPk(pool, collector, entries, chunkIdx, topK, groupByOpts,
				hasGroupBy, groupByFieldName, cols)
		} else {
			resultSources = mergeChunkInt64Pk(pool, collector, entries, chunkIdx, topK, groupByOpts,
				hasGroupBy, groupByFieldName, cols)
		}
...
func mergeChunkInt64Pk(
	...
	if hasGroupBy {
		gbArr := pickGroupByValues(pool, cols, groupByFieldName, chunkIdx, sources)
...
func mergeGroupByInt64Pk[GK comparable](
	...
	counter := newGroupCounter[GK](topK)
	...
	for int64(len(ids)) < totalLimit && h.Len() > 0 {
		...
		if !counter.shouldAccept(value, isNull, topK, groupSize) {
			if e.advance() {
				heap.Push(h, e)
			}
			continue
...
func newGroupCounter[K comparable](capacity int64) *groupCounter[K] {
	return &groupCounter[K]{
		groups: make(map[K]int64, capacity),
	}
}
...
func (c *groupCounter[K]) shouldAccept(value K, isNull bool, topK, groupSize int64) bool {
	var current int64
	var exists bool
	if isNull {
		current = c.nullCount
	...
	distinct := int64(len(c.groups))
	if c.nullCount > 0 {
		distinct++
	}
	if distinct >= topK && !exists {
		return false
	}
