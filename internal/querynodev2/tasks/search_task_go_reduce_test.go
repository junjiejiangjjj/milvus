...
func runGoReducePipeline(t *testing.T, ts *testSegments) (*mergeResult, []*chain.DataFrame) {
	...
	for _, res := range ts.searchResults {
		reader, err := segcore.ExportSearchResultAsArrowStream(context.Background(), res, plan, nil)
		require.NoError(t, err)
...
func TestExportSearchResultAsArrowStream_NoCMemoryLeak(t *testing.T) {
	...
	exportOnce := func() {
		for _, res := range ts.searchResults {
			reader, err := segcore.ExportSearchResultAsArrowStream(context.Background(), res, ts.searchReq.Plan(), extraFieldIDs)
			require.NoError(t, err)
			releaser, ok := reader.(interface{ Release() })
			require.True(t, ok)
			releaser.Release()
		}
	}
	...
	assert.Less(t, growth, int64(maxAllowedGrowth),
		"jemalloc allocated grew %d bytes over %d export iterations — likely C stream leak", growth, iterations)
