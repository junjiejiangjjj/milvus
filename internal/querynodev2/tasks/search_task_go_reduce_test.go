...
func TestExecuteMergedSubTasks(t *testing.T) {
	...
	for i, nq := range subTaskNqs {
		...
		require.NotNil(t, res, "sub-task %d: result must be populated", i)

		assert.Equal(t, nq, res.NumQueries, "sub-task %d", i)
		assert.Equal(t, topK, res.TopK, "sub-task %d", i)
		assert.NotEmpty(t, res.SlicedBlob, "sub-task %d", i)

		slice := &schemapb.SearchResultData{}
		require.NoError(t, proto.Unmarshal(res.SlicedBlob, slice))

		assert.Equal(t, nq, slice.NumQueries, "sub-task %d", i)
		assert.Equal(t, topK, slice.TopK, "sub-task %d", i)

		expectedRows := int(nq * topK)
		intIDs, ok := slice.Ids.IdField.(*schemapb.IDs_IntId)
		require.True(t, ok, "sub-task %d: expected int64 PKs", i)
		assert.Len(t, intIDs.IntId.Data, expectedRows, "sub-task %d", i)
		assert.Equal(t, expectedRows, len(slice.Scores), "sub-task %d", i)

		wantTopks := make([]int64, nq)
		for k := range wantTopks {
			wantTopks[k] = topK
		}
		assert.Equal(t, wantTopks, slice.Topks, "sub-task %d", i)
	}
...
func TestExecuteMergedSubTasks_MixedTopK(t *testing.T) {
	...
	require.NoError(t, receiver.executeGoReduce(segDFs, ts.searchResults, ts.searchReq, "IP", tr, 0))
