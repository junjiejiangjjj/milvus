...
func (t *SearchTask) executeGoReduce(
	...
	for i := range t.originNqs {
		...
		if err := t.buildSlicedResult(i, nqOffset, nq, groupSize, reduceResult, results, plan, metricType, tr, relatedDataSize); err != nil {
			return err
		}
		nqOffset += nq
	}
...
// fixupGroupByFieldType corrects the GroupByFieldValue proto type for field
// types that the Arrow layer cannot represent natively. The C++ Arrow exporter
// maps GEOMETRY (binary in proto, std::string in variant) to utf8 so the Go
// exporter produces VarChar/StringData. This function converts it to the
// correct Geometry/GeometryData to match the master reduce behavior.
func fixupGroupByFieldType(data *schemapb.SearchResultData, schema *schemapb.CollectionSchema) {
	gbv := data.GetGroupByFieldValue()
	if gbv == nil {
		return
	}
	fieldID := gbv.GetFieldId()
	if fieldID == 0 {
		return
	}
	for _, f := range schema.GetFields() {
		...
		if f.GetDataType() == schemapb.DataType_Geometry {
