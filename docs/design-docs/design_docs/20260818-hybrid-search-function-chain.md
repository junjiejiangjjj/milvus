...
the top-level Hybrid reranker. They are orthogonal to the top-level rerank source:
a request may use per-sub-search L0/L1 with a legacy ranker, typed
`function_score`, or a top-level L2 Function Chain.
...
- Preserve each nested chain in its sub-search plan and let QueryNode perform
  the existing L0/L1 validation before executing that chain.
...
This exclusivity applies only to the top-level
`HybridSearchRequest.function_chains`. Nested L0/L1 chains transform the scores
of their own ANN source before fusion and are therefore valid with every
top-level rerank source.
...
Per-sub-search L0/L1 may be combined with a top-level legacy ranker, typed
`function_score`, or L2 Function Chain. Nested `function_score` behavior is not
changed by this extension; this change neither implements new handling nor adds
a new rejection for it.
...
the compact request's chains into the matching
`PlanNode.querynode_function_chains`; QueryNode receives and validates the
execution contract through that plan.

### Per-sub-search plan generation

For each `SubReqs[i]`, Proxy assigns `SubReqs[i].function_chains` directly to
`plan.QuerynodeFunctionChains` before `marshalPlanWithMembershipFilterSizeLimit`.
Proxy does not perform L0/L1 semantic validation for nested chains.
...
QueryNode remains the authoritative validation boundary. Its preparation also
turns validated field names into field IDs and runtime state.

Proxy does not semantically validate nested L0/L1 chains before dispatch.
QueryNode normally prepares and validates them after Segcore has returned the
per-segment result slice and before Arrow export or chain execution. The
existing no-result shortcut is intentionally earlier: when `len(results) == 0`,
QueryNode returns an encoded empty result without preparing, validating, or
executing the nested chain. An invalid nested chain may therefore remain
unobserved on that path. If the result slice is present but contains zero
candidate rows, QueryNode still reaches chain preparation and validation.
...
- `function_chains` is combined with legacy `strategy`/`params` or typed `function_score`;
...
- a nested sub-search repeats L0 or L1, contains a nil/empty chain, or uses an
  operator/function unsupported at that stage;
...
3. Keep the ordinary Search L0/L1 structural, stage, function, system-column,
   and schema-input checks in QueryNode as the authoritative validation path.
4. Copy nested chains into their corresponding serialized plans without Proxy
   semantic validation. QueryNode rejects nested L2, Merge, and duplicate stages.
...
7. Keep the QueryNode execution and authoritative validation implementation
   unchanged.
8. Add REST nested `functionChains` parsing on `SubSearchReq` and attach the
   generated chains to the corresponding public `milvuspb.SearchRequest`.
9. Add Go unit, REST, mixed-stage, struct-element, validation, and old-client
...
- Proxy preserves malformed nested chains in the plan; QueryNode rejects them
  through its existing L0/L1 validation path before executing that chain.
- Old ranker without a chain remains unchanged.
- `function_score` plus a chain is rejected.
...
- Two sub-searches over the same vector field use different chains without
  cross-assignment.
- Nested L0/L1 works with each top-level source: legacy RRF/weighted ranker,
  typed `function_score`, and declarative L2 Merge.
