...
class TestFunctionChain(TestMilvusClientV2Base):
    ...
    def test_hybrid_search_with_l0_l1_l2_function_chains_execute_in_stage_order(self):
        ...
        l0_chain = FunctionChain(
            FunctionChainStage.L0_RERANK, name="hybrid_l0_add_ts"
        ).map(
            "$score",
            fn.num_combine(col("$score"), col(self.scalar_field), mode="sum"),
        )
    ...
    @pytest.mark.parametrize(
        "top_source",
        ["legacy_rrf", "function_score", "l2_function_chain"],
        ids=["legacy-rrf", "function-score", "l2-function-chain"],
    )
    def test_hybrid_search_nested_l0_l1_supports_each_top_level_rerank_source(
        self,
        top_source,
    ):
        """
        target: test nested L0/L1 is compatible with every top-level rerank source
        method: run the same nested chains with legacy RRF, typed decay FunctionScore,
                and a declarative L2 merge chain
        expected: all three sources successfully return a complete top-three result
        """
        client = self._client()
        collection_name = self._create_l1_function_chain_collection(client)
        ...
        if top_source == "legacy_rrf":
            ranker = RRFRanker()
        elif top_source == "function_score":
            ranker = Function(
                name="hybrid_decay",
                function_type=FunctionType.RERANK,
                input_field_names=[self.scalar_field],
                output_field_names=[],
                params={
                    "reranker": "decay",
                    "function": "gauss",
    ...
    def test_hybrid_search_with_l0_l1_l2_rejects_invalid_chain_placement_or_execution(
        ...
        valid_l2 = FunctionChain(
            FunctionChainStage.L2_RERANK, name="valid_hybrid_l2"
        ).merge(strategy="sum", norm_score=False)
        ...
        for function_chains in sub_chains:
            ...
            sub_request.function_chains.extend(
                function_chain.to_proto() for function_chain in function_chains
            )
